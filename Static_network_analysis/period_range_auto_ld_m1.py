...
* Light protocol: triangular ramp matching m1.py
    — 6 h ramp-up → 6 h ramp-down (peak 26.62) → 12 h dark, tiled for Nday days.
    — Photoreceptor flags derived per-hour from Ired / Iblue.
* Integration: odeint hour-by-hour loop (same as m1.py) for Nday=20 days (480 h).
* Burn-in: first 240 h discarded as transient (replaces original 72-point cut).
* Dark-phase shading added to expression plots.
* eta1, eta2 excluded from scan.
* Pool size capped at min(8, cpu_count).
* Output directory: parameter_analysis_ld/
"""
...
# ---------------------------------------------------------------------------
# Light schedule — triangular ramp LD 12:12 (matching m1.py)
# ---------------------------------------------------------------------------
def build_light_schedule(Nday=20):
    """
    DH = 12 dark hours.
    LH = 6 ramp-up + 6 ramp-down, peak = 26.62.
    Returns (IntensityRR, IntensityBB), each of length Nday*24.
    """
    DH, I_peak = 12, 26.62
    LH = (24 - DH) // 2
    one_day = np.concatenate((
        np.linspace(1, I_peak, LH),
        np.linspace(I_peak, 1, LH),
        np.zeros(DH),
    ))
    schedule = np.tile(one_day, Nday)
    return schedule, schedule.copy()


# ---------------------------------------------------------------------------
# ODE RHS factory — returns an odeint-compatible rhs(C, t)
# ---------------------------------------------------------------------------
def _make_rhs(params, IntensityRR, IntensityBB):
    eta1 = params["eta1"]
    eta2 = params["eta2"]

    def rhs(C, t):
        idx   = int(t) % len(IntensityRR)
        Ired  = IntensityRR[idx]
        Iblue = IntensityBB[idx]
...
# ---------------------------------------------------------------------------
# Simulation — hour-by-hour odeint loop (matching m1.py)
# ---------------------------------------------------------------------------
def run_simulation(params, Nday=20):
    ...
    rhs = _make_rhs(params, IntensityRR, IntensityBB)
    ...
    C_arr = np.array(C_all).T          # (19, Nday*24 + 1)
...
COMPONENTS      = [0, 2, 4, 6]
DARK_START_H    = 12   # first dark phase onset (h into analysis window)
...
def analyze_parameter(args):
    ...
    try:
        ...
        for val in adaptive_param_range(base_value, step_factor=1.1, max_fold=10):
            ...
            # --- Expression plot (2×2) with dark-phase shading ---
            t_max = t_crop[-1] if len(t_crop) > 0 else 240
            fig, axs = plt.subplots(2, 2, figsize=(10, 8), sharex=True)
            fig.suptitle(f"LD 12:12 — {param} = {val:.4g}", fontsize=11)
...
def main():
    ...
    results_dir = "parameter_analysis_ld"
    os.makedirs(results_dir, exist_ok=True)

    n_cores   = min(8, mp.cpu_count())
    task_args = [(k, v, results_dir) for k, v in param_items]

    print(f"M1 LD 12:12 parameter-range scan — {len(task_args)} parameters, {n_cores} workers")
