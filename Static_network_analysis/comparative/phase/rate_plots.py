...
def plot_slope_bars_dual(merged, title, outfile, y_label):
    ...
    plt.legend(fontsize=7, frameon=False, ncol=2, loc="upper center")
    plt.tight_layout()
...
def plot_dot(merged, title, outfile, y_label):
    ...
    plt.ylabel(y_label)
    plt.title(title)
    plt.legend(frameon=False, ncol=2, loc="upper center", fontsize=7)
    plt.tight_layout()
    plt.savefig(outdir / outfile, dpi=300)
    plt.close()
...
def plot_dumbbell(merged, title, outfile, y_label, reverse=True):
    set_pub_style()
    items = sorted(
        merged.items(),
        key=lambda item: item[1]["ll"] if np.isfinite(item[1]["ll"]) else -np.inf,
        reverse=reverse,
    )
    params = [p for p, _ in items]
    ll_vals = [d["ll"] for _, d in items]
    ...
    x = np.arange(len(params))
    dynamic_width = min(10.5, max(9.5, len(params) * 0.34))
    fig, ax = plt.subplots(figsize=(dynamic_width, 7.9))

    for i, (ll, ld) in enumerate(zip(ll_vals, ld_vals)):
        if np.isfinite(ll) and np.isfinite(ld):
            ax.plot([x[i], x[i]], [ll, ld], color="#999999", linewidth=1, alpha=0.6, zorder=1)

    ax.scatter(x, ll_vals, color=LL_COLOR, label="LL", zorder=3, s=18)
    ax.scatter(x, ld_vals, color=LD_COLOR, label="LD", zorder=3, s=18)
    ax.axhline(0, color="#bbbbbb", linewidth=0.6)

    ax.set_xticks(x)
    ax.set_xticklabels(params, rotation=90, ha="center", va="top")
    ax.set_ylabel(y_label)
    ax.set_xlabel("Parameter")
    ax.set_title(title, pad=10)
    ax.grid(axis="y", linestyle="--", linewidth=0.5, alpha=0.6)
    ax.legend(frameon=True, loc="upper center")
    ax.set_xlim(-0.45, len(params) - 0.55)
...
plot_dumbbell(
    ecc_merged,
    "Mean Eccentricity vs Fold Change",
