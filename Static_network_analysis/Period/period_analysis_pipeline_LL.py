#!/usr/bin/env python3
"""
period_analysis_pipeline_LL.py

COMBINED PIPELINE — Period Sensitivity Analysis | LL (Constant Light) condition
================================================================================
...
LL-specific differences vs LD version:
  • CONDITION_LABEL = "LL"  → stamped on all plot titles
  • OUTPUT_DIR_NAME = "LL_period_outputs"
  • BASE_PARAMS: all dark-cycle rate constants zeroed
      k1D, d2D, d3D, d5D, d6D → 0.0  (no dark phase under LL)
  • Expression profile line colours use a warm orange/amber palette
    to visually distinguish LL outputs from LD outputs at a glance.

All outputs land in:  <script_dir>/LL_period_outputs/

Run:
  python period_analysis_pipeline_LL.py
"""
...
# ──────────────────────────────────────────────────────────────────────────────
# Configuration — LL condition
# ──────────────────────────────────────────────────────────────────────────────
CONDITION_LABEL  = "LL"
OUTPUT_DIR_NAME  = "LL_period_outputs"

BASE_DIR  = Path(__file__).resolve().parent
DATA_DIR  = BASE_DIR / "parameter_analysis_ll"
OUT_DIR   = BASE_DIR / OUTPUT_DIR_NAME

COMPONENTS = ["LHYm", "P97m", "P51m", "ELm"]

# LL base parameter values
# Dark-cycle degradation rates zeroed; all other values identical to LD.
...
# Warm palette for LL — visually distinct from the cool LD palette
OKABE_ITO = [
    "#E69F00", "#D55E00", "#CC79A7", "#F0E442",
    "#009E73", "#56B4E9", "#0072B2", "#000000",
]
...
def step1_load_and_analyse() -> pd.DataFrame:
    ...
    for folder in sorted(DATA_DIR.glob("*/")):
        ...
        for comp in COMPONENTS:
            ...
            ax.plot(df[param_col], df[comp], marker="o", linewidth=1.6,
                    color=OKABE_ITO[0])   # warm orange for LL
    ...
    sns.heatmap(heatmap_df[COMPONENTS], annot=True, fmt=".1f",
                cmap="YlOrRd",   # warm colormap for LL
