#!/usr/bin/env python3
"""
phase_pipeline_LD.py

COMBINED PIPELINE — LD (Light/Dark) condition
=============================================
...
Outputs (all next to this script under  LD_outputs/):
  LD_outputs/phase_portraits/<param>/phase_<param>_<value>.png
  LD_outputs/norm_expression_profiles/<param>/exprnorm_<param>_<value>.png
  LD_outputs/area_ecc_analysis_<date>/
      subplot_metrics.csv
      mean_metrics.csv
...
Run:
  python phase_pipeline_LD.py
  python phase_pipeline_LD.py --root /path/to/parameter_analysis --drop_transient_frac 0.3
"""
...
# ──────────────────────────────────────────────────────────────────────────────
# Base parameter values for LD condition
# ──────────────────────────────────────────────────────────────────────────────
...
CONDITION_LABEL = "LD"        # used in titles and output folder names
OUTPUT_ROOT_NAME = "LD_outputs"
...
def parse_param_and_value(fp: Path) -> Tuple[Optional[str], Optional[float]]:
    name = fp.stem
    m = re.match(r"(?:expression_profile|expr)_(.+?)_([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)$", name)
    if m:
...
def plot_norm_expression(df: pd.DataFrame, param: str, value: float,
    ...
    comp_colors: Dict[str, str] = {
        "LHYm": "#0072B2", "P97m": "#D55E00", "P51m": "#009E73", "ELm": "#CC79A7",
    }
...
def run_plotting(root: Path, out_root: Path, glob_pat: str, drop_transient_frac: float) -> Path:
    """Run Step 1: generate all plots. Returns the phase portraits root directory."""
...
def main() -> None:
    ...
    parser.add_argument("--root", type=str, default=None,
                        help="Root directory containing parameter folders (default: <script_dir>/parameter_analysis_ld).")
    ...
    root = (script_dir / "parameter_analysis_ld") if args.root is None else Path(args.root).expanduser().resolve()
    if not root.exists():
