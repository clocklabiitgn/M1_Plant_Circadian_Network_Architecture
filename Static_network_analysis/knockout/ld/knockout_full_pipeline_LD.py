"""
knockout_full_pipeline_LD.py
════════════════════════════
Full sequential knockout analysis pipeline — LIGHT/DARK 12:12 (LD) condition.
...
# ─────────────────────────────────────────────────────────────────────────────
CONDITION          = "LD"
KNOCKOUT_CSV       = "knockout_results_ld/knockout_analysis_ld.csv"          # period-summary CSV
PROFILES_DIR       = Path("knockout_results_ld/expression_profiles")
OUTDIR             = Path("knockout_output_LD")
...
for col in ["Parameter", "Value"] + PERIOD_COLS:
...
for param, group in grouped:
    if param in FIXED_PARAMS:
        continue
    originals = group.loc[group["Value"] > 1e-6]
    knockouts = group.loc[group["Value"] <= 1e-6]
    ...
    core_sensitive_flags[param] = any(
        knockout[c] == 0 or pd.isna(knockout[c]) for c in PERIOD_COLS
    )
...
def infer_param_name(fp: Path) -> str:
    name = fp.name
    for suffix in ("_baseline_ld.csv", "_knockout_ld.csv", "_baseline.csv", "_knockout.csv"):
...
if not PROFILES_DIR.exists():
    ...
    if not baseline_files:
        print(f"  [skip] No '*_baseline*.csv' files found in {PROFILES_DIR.resolve()}")
    else:
        n_ok = 0
        for base_fp in baseline_files:
            param = infer_param_name(base_fp)
            ko_fp = next(
                (PROFILES_DIR / f"{param}{s}"
                 for s in ("_knockout_ld.csv", "_knockout.csv")
                 if (PROFILES_DIR / f"{param}{s}").exists()),
                None,
            )
            if ko_fp is None:
