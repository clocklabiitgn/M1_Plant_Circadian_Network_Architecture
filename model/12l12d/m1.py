...
intensity_pattern = np.concatenate((np.linspace(1, 26.62, LH), np.linspace(26.62, 1, LH), np.zeros(DH)))
...
ProteinLevel = []
C = 1*np.ones((1,19)) #initial conditions for all 19 variables
C[0,11] = 0
C[0,15] = 0
C[0,16] = 0
C[0,17] = 0
...
extracted_LHYm = LHYm[192:240]
extracted_P97m = P97m[192:240]
extracted_P51m = P51m[192:240]
extracted_ELm = ELm[192:240]
...
extracted_data.to_csv('model1_extracted_data.csv', index=False)
...
with open("model1_performance.txt", "w") as f:
    f.write("Model M1.1_red& blue:\n")
...
plot_expression_panel(final_LHYm, CCA1, LHY, ["M1", "CCA1 exp", "LHY exp"], 'mCLm.png')
plot_expression_panel(final_P51m, PRR5, TOC1, ["M1", "PRR5 exp", "TOC1 exp"], 'mP51m.png')
plot_expression_panel(final_P97m, PRR9, PRR7, ["M1", "PRR9 exp", "PRR7 exp"], 'mP97m.png')
plot_expression_panel(final_ELm, ELF4, LUX, ["M1", "ELF4 exp", "LUX exp"], 'mELm.png')
