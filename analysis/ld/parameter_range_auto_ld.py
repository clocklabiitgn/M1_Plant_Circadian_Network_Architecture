...
def run_simulation(params, t_span=(0, 240), dt=1.0, light_period=12.0, dark_period=12.0, print_light_cycle=False):
    def model(t, C):
        cycle = light_period + dark_period
        t_in_cycle = t % cycle
        light_on = t_in_cycle < light_period
        ThetaPhyA = ThetaPhyB = ThetaCry1 = 1.0 if light_on else 0.0
        Ired = Iblue = 26.62 if light_on else 0.0
        eta1, eta2 = 0.03, 0.0215
        #print("ThetaPhyA:", ThetaPhyA, "ThetaPhyB:", ThetaPhyB, "ThetaCry1:", ThetaCry1, "Ired:", Ired, "Iblue:", Iblue, file=sys.stderr)
    ...
    if print_light_cycle:
        cycle = light_period + dark_period
        light_on_series = (t_eval % cycle) < light_period
        for t, is_light in zip(t_eval, light_on_series):
            print(f"t={t:.2f} -> {'light' if is_light else 'dark'}")
...
def analyze_parameter(param, base_value, components, results_dir, light_period, dark_period, print_light_cycle):
    try:
        ...
        for val in param_gen:
            ...
            params[param] = val
            time, simulated_data = run_simulation(
                params,
                light_period=light_period,
                dark_period=dark_period,
                print_light_cycle=print_light_cycle,
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Parameter scan with light/dark cycling.")
    parser.add_argument("--light-period", type=float, default=12.0, help="Light duration per cycle in time units.")
    parser.add_argument("--dark-period", type=float, default=12.0, help="Dark duration per cycle in time units.")
    parser.add_argument("--print-light-cycle", action="store_true", help="Print light/dark state each timestep.")
    ...
    with mp.Pool(processes=60) as pool:
        tasks = [(param, base_value, components, results_dir, args.light_period, args.dark_period, args.print_light_cycle)
